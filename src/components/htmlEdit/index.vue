...
<script>
  ...
  export default {
    props:{
      ...
      uploadUrl:{
        type: String,
        default:"/apidev/upload/uploadFile"
      },
      /*上传图片的file控件name*/
      fileName:{
        type: String,
    ...
    methods: {
      onChange(){
        this.$emit('htmlinput', this.content)
      },
      /*选择上传图片切换*/
      onFileChange(e){
        var self=this;
        ...
        axios.post(this.uploadUrl,data).then(function(res){
          console.log(res);
          if(res.data) {
            self.editor.insertEmbed(self.editor.getSelection().index, 'image', res.data.retObj.picPath);
          }
        })
      },
      /*点击上传图片按钮*/
    ...
    computed:{
      ...
      fileId(){
        return `textWrapper${this.id}`
      }
    },
    mounted(){
//      this.content=this.value;
      var that = this
      let text = document.getElementById(that.fileId);
      text.style.width = this.size[0]+'px';
      text.style.height = this.size[1]+'px';
