...
//  import $ from 'jquery'
  ...
  export default {
    ...
    methods:{
      ...
      getfunlist() {
        this.$fetch.api_power.getfunlist({
          ...
          if(retcode==0){
            ...
            var zTree = $.fn.zTree.init($("#tree"), setting, that.treeData);
            zTree.expandAll(false)
            var nodes = zTree.getNodes()
            var array = zTree.transformToArray(nodes)
            this.$fetch.api_power.getRoleFun({
              id: this.$route.params.id
            }).then(({msg, retcode, retObj})=> {

              for(var i=0; i<retObj.length; i++) {
                var node = zTree.getNodeByParam("id", retObj[i].id);
                try{zTree.checkNode(node, true, false);}catch(e){}
                zTree.expandNode(node, true); //展开选中的
              }
            })
          }
        })
      ...
      getRoelDetil() {
        this.$fetch.api_power.getRoleDetil({
          id: this.$route.params.id
        }).then(({msg, retcode, retObj})=> {
          this.modusDetail.id = retObj.id
          this.modusDetail.title = retObj.title
        })
      },
    },
    created() {
