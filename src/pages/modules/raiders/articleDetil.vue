  ...
  <div class='panel'>
    ...
    <div class="panel-body tuijList">
      ...
      <div :class="`tjImg${index}`" v-for="(item,index) in tuijian" style="margin:10px 0;">
        <div class="tjWrapper">
          <span style="margin-right: 10px">推荐商品{{index+1}}</span><el-input style="width: 250px;"></el-input>
        </div>
        <div>
          <pics-upload :defaultImgs="defaultImgs[index]" :id="index" :postImgUrl="postImgUrl" :maxPostImgs="1"></pics-upload>
        </div>
      </div>
    </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      save() {
        this.articleDetail.area = this.getCityCode
        this.articleDetail.classify = this.classify
        ...
        this.$fetch.api_raiders.recommendAdd(this.articleDetail)
          .then(({msg, retcode, retObj})=> {
      ...
      tjSave() {
        ...
        ipts.each(function (i) {
          ...
          shopObj.recommendId = that.recommendId
    ...
    created() {
      ...
      this.$fetch.api_business.getNewsDetil({
        ...
        this.articleDetail = retObj
        this.defaultImgs = this.articleDetail.pic.split(',');
