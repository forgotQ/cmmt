  ...
  <div class="panel">
    <panel-title>
      <el-select class="classSlt" v-model="classType" placeholder="请选择分类类型" @change="switchClass">
        <el-option label="自由行" value="0"></el-option>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <el-form :model="classDetil">
        ...
        <el-form-item label="行程天数" :label-width="formLabelWidth">
          <el-input  auto-complete="on" v-model="classDetil.numberday"></el-input>
        </el-form-item>
      </el-form>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      sure() {
        this.classDetil.category = this.classType
        this.classDetil.area = this.getCityCode
        this.$fetch.api_raiders.numberdayAdd(this.classDetil)
          .then(({msg, retcode, retObj})=> {
            if (retcode == 0) {
              this.$message.success(msg)
            }
      ...
      submit() {
        this.$fetch.api_raiders.numberdayEdit(this.classDetil)
          .then(({msg, retcode, retObj})=> {
      ...
      delete_data(item){
        this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
          ...
          .then(() => {
             ...
             this.$fetch.api_raiders.numberdayDel({
               id: item
             }).then((data) => {
             this.get_class_data()
             this.$message.success(msg)
