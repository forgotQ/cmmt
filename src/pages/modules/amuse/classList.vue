  ...
  <div class="panel">
    <panel-title>
      <el-select class="classSlt" v-model="classType" placeholder="请选择分类类型" @change="switchClass">
        <el-option label="明星分类" value="3"></el-option>
        <el-option label="资讯分类" value="1"></el-option>
        <el-option label="明星学院分类" value="2"></el-option>
        <el-option label="探班分类" value="4"></el-option>
      </el-select>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <el-form :model="classDetil">
        ...
        <el-form-item label="分类描述" :label-width="formLabelWidth">
          <el-input type="textarea" v-model="classDetil.typeDesc"></el-input>
        </el-form-item>
        <el-form-item label="是否开启" :label-width="formLabelWidth">
          <el-radio class="radio" v-model="classDetil.delFlag" label="0">是</el-radio>
          <el-radio class="radio" v-model="classDetil.delFlag" label="2">否</el-radio>
        </el-form-item>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      sure() {
        this.classDetil.fun = this.fun
        this.classDetil.classify = this.classType
        this.$fetch.api_business.addClass(this.classDetil)
          .then(({msg, retcode, retObj})=> {
      ...
      submit() {
        this.$fetch.api_business.editClass(this.classDetil)
          .then(({msg, retcode, retObj})=> {
      ...
      delete_data(item){
        this.$confirm('此操作将删除该数据, 是否继续?', '提示', {
          ...
          .then(() => {
             ...
             this.$fetch.api_business.delClass({
               id: item
             }).then((data) => {
             console.log(data);
             this.get_class_data()
